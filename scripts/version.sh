#!/usr/bin/env bash
# The single source of this project's version. Nothing is declared in a file: releasing is tagging.
#
#   scripts/version.sh name   git describe against semver tags: v1.0.0, v1.0.0-3-gabc1234, or a bare
#                             hash before the first one
...
#
# Outside git the name is "unknown" and the code 0, and so is the code in a shallow clone, whose commit
# count would come out lower than the build before it.
set -euo pipefail
cd "$(dirname "$0")/.."
